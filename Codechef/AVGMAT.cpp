...
int main() {
  ...
  while (T--) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      for (int j = 0; j < M; j++) {
        ...
        if (cross_diagonal[i + j].size() == 0) {
          ...
          cross_diagonal[i + j].push_back(cross_diagonal[i + j].back() +
                                          mat[i][j]);
        }
        if (main_diagonal[i - j + 1000].size() == 0) {
          main_diagonal[i - j + 1000].push_back(mat[i][j]);
        } else {
          main_diagonal[i - j + 1000].push_back(
              main_diagonal[i - j + 1000].back() + mat[i][j]);
        }
      }
    }
    ...
    for (auto& p : ones) {
      ...
      for (int d = 1; d <= N + M - 2; d++) {
        ...
        if (sz1 > 0) {
          ...
          if (v(right.first, right.second))
            high = main_idx[right.first][right.second];
        ...
        if (sz2 > 0) {
          ...
          if (v(bottom.first, bottom.second))
            high = main_idx[bottom.first][bottom.second];
          if (v(left.first, left.second))
            low = main_idx[left.first][left.second];
        ...
        if (e3 >= 0) {
          ...
          if (sz3 > 0) {
            ...
            if (v(left.first, left.second))
              high = cross_idx[left.first][left.second];
            if (v(top.first, top.second))
              low = cross_idx[top.first][top.second];
        ...
        if (e4 >= 0) {
          ...
          if (sz4 > 0) {
            ...
            if (v(bottom.first, bottom.second))
              high = cross_idx[bottom.first][bottom.second];
            if (v(right.first, right.second))
              low = cross_idx[right.first][right.second];
            // cout << "low => " << low << '\n';
            // cout << "high =>" << high << '\n';
            ...
            // cout << p << if (v(top.first, top.second) &&
            // mat[top.first][top.second]) val--;" => " << d << " :: " << val <<
            // '\n';
          }
        }
        ...
        if (v(bottom.first, bottom.second) && mat[bottom.first][bottom.second])
          cnt--;

        if (v(left.first, left.second) && mat[left.first][left.second]) cnt--;
        if (v(right.first, right.second) && mat[right.first][right.second])
          cnt--;
    ...
    for (int i = 1; i <= N + M - 2; i++) cout << D[i] / 2 << ' ';
    cout << '\n';
  }
}
