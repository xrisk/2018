			...
			for (int size = 1; size <= min(row, col); size++) {
				...
				if (size == 1) {
					...
					if ((i + size - 1) % 2 == 0) { 
						...
						dp_black[i][j][size] += cum_col_black[j + size - 1][i + size - 1]
											 - (i == 0 ? 0 : cum_col_black[j + size - 1][i - 1]);
						dp_white[i][j][size] += cum_col_white[j + size - 1][i + size - 1]
											 - (i == 0 ? 0 : cum_col_white[j + size - 1][i - 1]);
					} else {
						// if (i == 0 && j == 0 && size == 2) cout << "here";
						dp_black[i][j][size] += cum_col_white[j + size - 1][i + size - 1]
											  - (i == 0 ? 0 : cum_col_white[j + size - 1][i - 1]);
						dp_white[i][j][size] += cum_col_black[j + size - 1][i + size - 1]
											  - (i == 0 ? 0 : cum_col_black[j + size - 1][i - 1]);
					}

					if ((j + size - 1) % 2 == 0) { 
						// first cell of col is going to be same color as corner cell
						dp_black[i][j][size] += cum_row_black[i + size - 1][j + size - 1]
											 - (j == 0 ? 0 : cum_row_black[i + size - 1][j - 1]);
						dp_white[i][j][size] += cum_row_white[i + size - 1][j + size - 1]
											 - (j == 0 ? 0 : cum_row_white[i + size - 1][j - 1]);
					} else {
						// if (i == 0 && j == 0 && size == 2) cout << "here";
						dp_black[i][j][size] += cum_row_white[i + size - 1][j + size - 1]
											  - (j == 0 ? 0 : cum_row_white[i + size - 1][j - 1]);
						dp_white[i][j][size] += cum_row_black[i + size - 1][j + size - 1]
											  - (j == 0 ? 0 : cum_row_black[i + size - 1][j - 1]);
					}
